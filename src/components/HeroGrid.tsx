...
const HeroGrid: React.FC<HeroGridProps> = ({
  ...
  const roles = ["Carry", "Support", "Nuker", "Disabler", "Durable", "Initiator"];

  const primaryAttrs = [
    { key: "str", label: "Strength", color: "text-red-500", bg: "bg-red-500/5" },
    { key: "agi", label: "Agility", color: "text-green-500", bg: "bg-green-500/5" },
    { key: "int", label: "Intelligence", color: "text-blue-400", bg: "bg-blue-400/5" },
    { key: "all", label: "Universal", color: "text-purple-400", bg: "bg-purple-400/5" },
  ];

  const filteredHeroes = heroes.filter((hero) => {
    const matchesSearch = hero.localized_name.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="bg-slate-900/40 backdrop-blur-md rounded-lg border border-slate-800 shadow-2xl overflow-hidden">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 no-scrollbar">
            <button
              ...
              className={`whitespace-nowrap px-3 py-1.5 rounded text-[10px] font-black uppercase tracking-tighter transition-all border ${
                selectedRole === null ? "bg-white text-black border-white" : "bg-slate-950 border-slate-800 text-slate-500 hover:text-slate-300"
              }`}
            >
              All Roles
            </button>
            {roles.map(role => (
              <button
                ...
                className={`whitespace-nowrap px-3 py-1.5 rounded text-[10px] font-black uppercase tracking-tighter transition-all border ${
                  selectedRole === role ? "bg-blue-600 text-white border-blue-500 shadow-[0_0_10px_rgba(37,99,235,0.3)]" : "bg-slate-950 border-slate-800 text-slate-500 hover:text-slate-300"
                }`}
              >
                {role}
              </button>
      ...
      <div className="p-4 md:p-6 space-y-10 max-h-[550px] overflow-y-auto pr-2 custom-scrollbar">
        {primaryAttrs.map((attr) => {
          const attrHeroes = filteredHeroes.filter(h => h.primary_attr === attr.key);
          if (attrHeroes.length === 0) return null;

          return (
            <div key={attr.key} className={`p-4 rounded-lg ${attr.bg} border border-white/5`}>
              <div className="flex items-center gap-3 mb-4">
                <div className={`w-1 h-4 rounded-full ${attr.color.replace('text', 'bg')}`}></div>
                <h4 className={`text-[11px] font-black uppercase tracking-[0.2em] ${attr.color}`}>
                  {attr.label}
                </h4>
              </div>
                  ...
                  return (
                    <div
                      ...
                      className={`relative group aspect-[16/9] overflow-hidden rounded shadow-xl transition-all duration-300 ${
                        isSelected ? "opacity-10 grayscale scale-90 border-transparent pointer-events-none" : "hover:scale-110 hover:z-20 cursor-pointer border border-white/10"
                      }`}
                    >
                      <img
                      ...
                      <div className="absolute inset-0 flex opacity-0 group-hover:opacity-100 transition-all duration-300 z-10 translate-y-2 group-hover:translate-y-0">
                        ...
                        >
                          <Plus size={14} className="text-white drop-shadow-lg" strokeWidth={3} />
                          <span className="text-[7px] font-black text-white uppercase mt-1">Pick Us</span>
                        </div>
                        <div 
                        ...
                        >
                          <Plus size={14} className="text-white drop-shadow-lg" strokeWidth={3} />
                          <span className="text-[7px] font-black text-white uppercase mt-1">Pick Em</span>
                        </div>
                      </div>
                    </div>
                  );
